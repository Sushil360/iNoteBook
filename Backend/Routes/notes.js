...
router.put("/updatenote/:id", fetchuser, async (req, res) => {
  ...
  try {
    ...
    //firstly it checks wheather the note you want to update exist or not
    let note = await Note.findById(req.params.id); //this id is the id given in the /updatenote/:id
    if (!note) {
      return res.status(404).send("Note Not Found");
    }

    //checks wheather the note truely belongs to the right user
